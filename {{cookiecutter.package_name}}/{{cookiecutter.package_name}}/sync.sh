...
CURRENT_PATH=`pwd`
CATALOGNAME="{{ cookiecutter.package_name }}"
...
while [[ $PWD != / ]] ; do
    if [ -e "bin/i18ndude" ]; then
        I18NDUDE="$PWD/bin/i18ndude"
    fi
    cd ..
done
...
# Rebuild .pot
$I18NDUDE rebuild-pot --exclude "" --pot locales/$CATALOGNAME.pot $MERGE --create $CATALOGNAME .
